...
public class StudentController {
    ...
    public ResponseEntity<Student> getStudentByRollNo(@PathVariable("id") long id) {
        Student student = studentService.studentMap.get(id);
        if (student != null) {
    ...
    public ResponseEntity<String>updateByName(@PathVariable("name") String name,@RequestBody Student student){
        Student student1=studentService.updateByName(name,student);
        if(student1 !=null){
            ResponseEntity.ok(student1);
        }

        return ResponseEntity.status(HttpStatus.NOT_FOUND).body("Student with name '" + name + "' not found or not updated");
    }
